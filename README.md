...
- **Structural proof-of-work** — each iteration must include a `codex_response_hash` (sha256 of actual Codex output). The hook verifies these exist. Claude cannot fake verification without actually calling Codex.
- **Cross-checked state** — the hook validates that iteration logs match state file claims. Claude cannot inflate iteration counts or fake agreement history.
...
### What the stop hook now verifies

```mermaid
flowchart TD
    A[Non-trivial task tries to stop] --> B{verify-log.jsonl exists?}
    B -->|No| BLOCK1["BLOCK: No iteration log"]
    B -->|Yes| C{Any entries with codex_response_hash?}
    C -->|No| BLOCK2["BLOCK: Codex never called"]
    C -->|Yes| D{Log iteration count >= state iteration count?}
    D -->|No| BLOCK3["BLOCK: Iteration count mismatch"]
    D -->|Yes| E{iteration >= 6?}
    E -->|Yes| ALLOW1[ALLOW: capped]
    E -->|No| F{2 consecutive agreements in state?}
    F -->|No| BLOCK4["BLOCK: Not enough agreements"]
    F -->|Yes| G{Last 2 log entries also agreed=true?}
    G -->|No| BLOCK5["BLOCK: State/log mismatch"]
    G -->|Yes| ALLOW2["ALLOW: Aegean stable"]

    style BLOCK1 fill:#9d0208,color:#fff
    style BLOCK2 fill:#9d0208,color:#fff
    style BLOCK3 fill:#9d0208,color:#fff
    style BLOCK4 fill:#9d0208,color:#fff
    style BLOCK5 fill:#9d0208,color:#fff
    style ALLOW1 fill:#2d6a4f,color:#fff
    style ALLOW2 fill:#2d6a4f,color:#fff
```

### Proof-of-work: `codex_response_hash`

Each iteration log entry must include a `codex_response_hash` — the first 8 characters of the sha256 hash of the raw Codex response:

```bash
# Generated by Claude after receiving Codex output
CODEX_HASH=$(echo -n "<raw codex response>" | sha256sum | cut -c1-8)
```

This is verified at two points:
1. **Iteration logger** (`log-verify-iteration.sh`) — rejects iteration events without the hash
2. **Stop hook** (`stop-verify-gate.sh`) — blocks if no iteration entries have hashes

### What Claude cannot do

| Bypass attempt | Why it fails |
|---------------|-------------|
| Write `trivial: true` on a real task | Gate logs show this — auditable by user |
| Fake iteration history in state file | Hook cross-checks state against iteration log |
| Skip Codex calls | No `codex_response_hash` in log = BLOCK |
| Inflate iteration count | Log entry count must match state claims |
| Claim agreement when Codex disagreed | Hook verifies last 2 log entries match state |
